...
void main() {
  ...
  group('reconcile', () {
    ...
    test('a per-view minimum below the uniform floor keeps a smaller '
        'persisted size', () {
      const persisted = WorkbenchLayoutState(
        sizes: {
          'explorer': {'short': 60.0, 'tiny': 10.0},
        },
      );
      final reconciled = persisted.reconcile({
        'explorer': [
          _view('short', minimumBodySize: 40.0),
          _view('tiny', minimumBodySize: 40.0),
        ],
      });
      expect(reconciled.sizes['explorer'], {'short': 60.0, 'tiny': 40.0});
    });
